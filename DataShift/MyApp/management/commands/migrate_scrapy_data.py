...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        with scrapy_connection.cursor() as cursor:
            ...
            with transaction.atomic(using="default"):
                for hotel in hotels:
                    ...
                    full_address = hotel_data["fullAddress"][:100]
                    # Adjust max length as needed
                    hotel_name = hotel_data["hotelName"][:100]
                    # Adjust max length as needed
                    brief = hotel_data["brief"][:500]

                    # Create or get Location
                    location, created = Location.objects.using(
                        "default"
                    ).get_or_create(
                        name=full_address,
                        defaults={
                    ...
                    property_obj, created = Property.objects.using(
                        "default"
                    ).update_or_create(
                    ...
                    if hotel_data.get("hotelFacilityList"):
                        ...
                        for amenity_name in hotel_data["hotelFacilityList"]:
                            # Adjust max length as needed
                            amenity_name = amenity_name[:100]
                            amenity, created = Amenity.objects.using(
                                "default"
                            ).get_or_create(name=amenity_name)
                    ...
                    img_src = (
                        f"/media/hotel_images/{os.path.basename(img_src)}"
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Data and images migrated from Scrapy to Django successfully!"
            )
        )

    def migrate_images(self):
        scrapy_image_dir = (
            "/home/w3e55/w3_assignment/Assignment_5_modified/"
            "hotalInfo/hotel_images"
        )
        django_image_dir = os.path.join(
            settings.BASE_DIR, "media", "hotel_images"
        )
